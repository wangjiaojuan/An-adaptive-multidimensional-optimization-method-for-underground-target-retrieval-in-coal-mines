...
from D_weight_engineering_relation_function import*
import numpy as np
import time
import warnings
...
def compare(Datasets_name):
    file_path = './results/'+Datasets_name+"_weight.txt"
    ...
    for i in range(len(re)):
        ...
        if Datasets_name == 'CUMT-BelT':
            ...
            result_1=query_result(distance1,ac_label,database,Datasets_name)

            distance2=haming_distance(all_feature_2,path,database)
            result_2=query_result(distance2,ac_label,database,Datasets_name)

            distance3=haming_distance(all_feature_3,path,database)
            result_3=query_result(distance3,ac_label,database,Datasets_name)

            time1 = time.time()
            ours_ac,B = ours_query3(result_1,result_2,result_3,distance1,distance2,distance3,ac_label,database,Datasets_name)
            time3 = time.time()

            print(result_1,result_2,result_3,B[0],B[1],B[2],file=filewrite)
        ...
        if Datasets_name == 'mydataset':
            ...
            time1 = time.time()
            ours_ac,B = ours_query2(result_1,result_2,distance1,distance2,ac_label,database,Datasets_name)
            time3 = time.time()    
            print(result_1,result_2,B[0],B[1],file=filewrite)
            neet_time = neet_time + (time3 - time1)
            cal_ac = cal_ac+ours_ac
...
if __name__ == '__main__':
    for Datasets_name in ['mydataset']:#['mydataset']:#['CUMT-BelT']:
        #binary_datasets_feature('./features/')
